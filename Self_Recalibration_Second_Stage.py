...
def Training(fold = None , percentage = None , config = None , dataset = None , model = None):
    ...
    Sigma = tf.compat.v1.placeholder(dtype = tf.float32 , shape = [None , None , 1])
    ...
    with tf.compat.v1.Session() as sess:
        ...
        best_MAE = 2000
        best_MSE = 2000
        f = open("logs_second_stage_{0}_{1}_{2}.txt".format(model,dataset[5],str(fold)), "a")
        ...
        for epoch in range(Epoch+1):
            ...
            for (A_or_B ,  train_dir) in train_procedure:
                ...
                for r in range(number_of_batch):
                    ...
                    (X_train_batch , Y_train_batch) = Shanghai.data_loader_pipeline(A_or_B = A_or_B , train_or_test = train_dir , 
                                                                                    data_list = data_list[r*batch_SIZE:(r+1)*batch_SIZE], training_stage = "2")
            ...
            if epoch >= (Epoch//2) and epoch % 2 == 0:
                ...
                if test_MAE <= best_MAE:
                    best_MAE = test_MAE
                    best_MSE = test_MSE
                    Saver.save(sess,"./model_our_method_{0}_{1}_{2}/BLpp.ckpt".format(model , dataset[5] , str(fold)))
...
if __name__ == "__main__" :
    ...
    config = [False , "adap" , "V2" , [0.1 , 0.] , 2.0]
    


    for i in range(5): # 5-fold training

        if not os.path.isdir(".\\model_our_method_{0}_{1}_{2}".format(args.model , args.data_dir[5], str(i))):
            
            os.makedirs(".\\model_our_method_{0}_{1}_{2}".format(args.model , args.data_dir[5], str(i)))
