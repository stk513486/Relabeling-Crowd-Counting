...
def Training(fold = None , percentage = None , config = None , dataset = None, model = None):
    ...
    loss , decoupled_density , expectation = tools.Mixture_Bayesian_Loss(z , GT , score_scale ,
                                                                        BG = config[1] ,
                                                                        Version = config[2] ,
                                                                        delta = config[3],
                                                                        sigma = config[4])
    ...
    with tf.compat.v1.Session() as sess:
        ...
        best_MAE = 2000
        best_MSE = 2000
        f = open("logs_{0}_{1}_{2}.txt".format(model,dataset[5],str(fold)), "a")
        ...
        for epoch in range(Epoch+1):
            ...
            if epoch == first_write_epoch and first_write_flag == False and config[0] == True:
                ...
                for file in data_list:
                    ...
                    if np.sum(np.abs(dens-1)) <= np.sum(Y_train_batch[0][0][0,:,-1:,-1:]):
                        
                        relabel_tools.relabel_operation(A_or_B , file , dens , expects , False , Y_train_batch[0][0])
            ...
            if epoch == Epoch:
                ...
                Saver.save(sess,"./model_last_epoch_for_inference_{0}/BLpp.ckpt".format(str(fold)))
...
if __name__ == "__main__" :
    ...
    # don't change them
    config = [True ,            # True = turn on the relabeling mechanism
              "adap" ,          # ignore it
              "V2" ,            # ignore it
              [0.25 , 0.5] ,    # first element means the coefficient of the regularization loss, second one means the delta in the Hinge Loss
              2.0]              # sigma in the foreground likelihood probability   
   


    for i in range(5): # 5-fold training

        if not os.path.isdir(".\\model_last_epoch_for_inference_" + str(i)):
        
            os.makedirs(".\\model_last_epoch_for_inference_" + str(i))
