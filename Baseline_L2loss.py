...
def Training(fold = None , percentage = None , config = None , dataset = None, model = None):
    ...
    loss = tools.pixel_wise_l2_loss(z , GT)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        best_MAE = 200
        best_MSE = 200
        f = open("logs_baseline_L2loss_{0}_{1}_{2}.txt".format(model , dataset[5] , str(fold)) , "a")
        ...
        for epoch in range(Epoch+1):
            ...
            if epoch >= (Epoch//2) and epoch % 2 == 0:
                ...
                if test_MAE <= best_MAE:
                    best_MAE = test_MAE
                    best_MSE = test_MSE
                    Saver.save(sess,"./model_baseline_L2loss_{0}_{1}_{2}/BLpp.ckpt".format(model , dataset[5] , str(fold)))
...
if __name__ == "__main__" :
    ...
    for i in range(5):
        ...
        if not os.path.isdir(".\\model_baseline_L2loss_{0}_{1}_{2}".format(args.model , args.data_dir[5], str(i))):
            
            os.makedirs(".\\model_baseline_L2loss_{0}_{1}_{2}".format(args.model , args.data_dir[5], str(i)))
